...
contract NFTContract is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_MINTS_PER_ADDRESS = 1;
    ...
    mapping(address => uint256) vipAddressCanHaveNumber;
...
//    uint256 amount = 1;         Actual mint quantity = amount + 1
    function setVipAddressLimit(address[] memory addresses, uint256 amount) external onlyOwner {
    ...
    function mintNFT(string memory uri, bytes32[] memory proof) public payable nonReentrant {
        ...
        if (block.timestamp <= endTime) {
            if (checkIfUserIsWhitelisted(msg.sender, proof)) {
                ...
                uint256 amount =vipAddressCanHaveNumber[msg.sender];
                if (amount == 0) {
